...
let AdminService = class AdminService {
    ...
    async getDashboardStats(adminStatsDto) {
        ...
        const [bookingStats, paymentStats, driverStats, vehicleStats, userStats, recentBookings, activeBookings,] = await Promise.all([
    ...
    async assignDriver(assignmentDto) {
        ...
        switch (strategy) {
            case driver_assignment_dto_1.AssignmentStrategy.MANUAL:
                ...
                break;
            case driver_assignment_dto_1.AssignmentStrategy.NEAREST:
            ...
            case driver_assignment_dto_1.AssignmentStrategy.ROUND_ROBIN:
                ...
                break;
            case driver_assignment_dto_1.AssignmentStrategy.RATING_BASED:
        ...
        await this.trackingGateway.broadcastBookingStatusUpdate(bookingId, enums_1.BookingStatus.ASSIGNED, { driverName: driver.name, vehiclePlate: driver.vehicle?.licensePlate });
        return {
    ...
    async getSystemHealth() {
        const [totalBookings, activeBookings, availableDrivers, onlineDrivers, pendingPayments,] = await Promise.all([
            ...
            this.driversService.findAll().then(drivers => drivers.filter(d => d.status !== enums_1.DriverStatus.OFFLINE).length),
            this.paymentsService.getAllPayments(1, 1000).then(result => result.data.filter(p => p.status === 'pending').length),
        ]);
